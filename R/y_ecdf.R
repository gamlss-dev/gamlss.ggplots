...
if (missing(y))   stop("the first argument y is needed")
  ...
  gg <- ggplot2::ggplot() +
        ggplot2::geom_ribbon(data=dx, ggplot2::aes(ymin = lower, ymax = upper, x = x), 
                             alpha = 0.2)+
       ggplot2::stat_ecdf(data=d, ggplot2::aes(x = y, y = scores), geom = "step", 
                          color=points.col)+
       ggplot2::xlab(xlabel) + 
...
if  (show.outliers)  ggplot2::geom_text(data = d, ggplot2::aes(x = y, y = scores, 
                              label = txt),
