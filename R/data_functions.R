...
if (fit.method=="linear")
  ...
  p# you also need thedata as output 
}          
...
################################################################################
################################################################################
data_exclude <- function(data, what="factor")
{
if (is(data, "list"))  
    stop("the data is list  the function needs a data.frame") 
if (is(data, "table")) 
...
if (is(data, "array"))   
  ...
  CData <- sapply(data,class)
     da <- subset(data, select=CData!=what)
da
}  
################################################################################
################################################################################
################################################################################
################################################################################
data_remove <- function(data, what)
{
  if (is(data, "list"))  
    stop("the data is list  the function needs a data.frame") 
  if (is(data, "table")) 
  ...
  {
    da <- subset(data, select=setdiff(names(data), what))  
  } else
  {
    da <- subset(data, select=setdiff(1:dim(data)[2], what)) 
  }  
  da
}  
################################################################################
