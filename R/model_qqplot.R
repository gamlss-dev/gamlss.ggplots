    ...
    if (length(list(...)) > 0) 
    {
      ...
      for (resp in list(...)) 
      {
        ...
        if ((is(resp,"gamlss")))
        {
         ...
         resa <- data.frame(obs = obs, rqres=res, x=x, model=rep(names[[JJ]], length(rqres))) 
        } else
        {
          res <- residuals(resp) 
          obs <- seq_len(length(res))
          ...
          obs <- obs[!is.na(res)]
          x <- qnorm(ppoints(length(res)))[order(order(res))] 
          resa <- data.frame(obs = obs, rqres=res, x=x, model=rep(names[[JJ]], 
                                                                    length(res)))  
        } 
        out <- rbind(out, resa)
      }    
...
gg <- ggplot2::ggplot(d, aes(x = x, y = rqres, color = model)) + 
