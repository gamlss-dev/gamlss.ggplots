...
# ii)   ordering is DONE
# iii) there is s problem if names line "same-more"  in the data they need to
#      be "some_more" why? 
################################################################################
################################################################################
################################################################################
...
model2formula2data <- function(obj, data)
{
...
# PP <- unlist(lapply(formul,"length"))
# for (i in 1:length(PP)) 
# {
#   if  (PP==3) 
#   {
#     if (length(formul[[i]][[3]])>1)  formul[[i]] <- formul[[i]][[3]][[2]]
#   }
# }
...
# from a gamlss model get the folrulae and create a data frame
      DaTa <- model2formula2data(obj, DaTa) 
#if ((obj$family[1]%in%gamlss.bi.list[1]))
#      {
#        bd <- obj$bd
#        DaTa <- cbind(DaTa, bd)
#      }
...
DAT$names <- with(DAT, reorder(names, ri, median))
       gg <- ggplot2::ggplot(DAT, ggplot2::aes(names, ri))+
         ggplot2::geom_segment( aes(x=names, xend=names, y=0, yend=ri), color=colour)+
         ggplot2::geom_point( color="blue", size=point_size, alpha=point_alpha) + 
 ...
 DAT$names <- with(DAT, reorder(names, RI, median)) 
        gg <- ggplot2::ggplot(DAT, aes(names, ri))+
