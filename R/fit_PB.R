...
################################################################################ 
  ...
  yname <- deparse(substitute(y))
      ...
      w <- if (!missing(data))
       ...
       else w <- get(deparse(substitute(weights)), 
...
lambda.old <- lambda
    lambda <- sig2 / tau2 
    # maybe only 1/tau2 will do since it gives exactly the EM results see LM-1
...
if (plot)
  {
    ...
    gg <- ggplot2::ggplot(data=df, ggplot2::aes(x=x, y=y))+
      ...
      ggplot2::ylab(yname)+  ggplot2::xlab(xname)+
      ggplot2::geom_ribbon(data=df, ggplot2::aes(ymin = lower, ymax = upper, x = x), alpha=.5, fill=col.ribbon)
    print(gg)
  }
