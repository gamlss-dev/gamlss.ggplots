...
################################################################################
################################################################################
fitted_survival <- function (model,
                        obs,
                        title,
                      from = 0,
                        to = 10,
...
################################################################################      
txt.title <- if (missing(title))  
  paste("Fitted survival functions from model",deparse(substitute(model)))
      else title
################################################################################      
  MM <- if (is.gamlss(model)) {predictAll(model, output="data.frame")[obs,]}
         else                  {predict(model, type="parameter")[obs,] }
...
if (fname%in%gamlss.bi.list)  
  {
    ...
    for (i in 1:lobs)
    {
      ...
      switch(nopar,
             {
        cdfArr[[i]] <- 1-cdf(y.var[[i]],  mu=MM[i,"mu"],  bd=MM[i,"bd"])
             },  
             {
        cdfArr[[i]] <- 1-cdf(y.var[[i]], mu=MM[i,"mu"],  sigma=MM[i,"sigma"], bd=MM[i,"bd"])
             },
             {
       cdfArr[[i]] <- 1-cdf(y.var[[i]],  mu=MM[i,"mu"], sigma=MM[i,"sigma"], nu=MM[i,"nu"], bd=MM[i,"bd"])
             },
             {
      cdfArr[[i]] <- 1-cdf(y.var[[i]],  mu=MM[j,"mu"], sigma=MM[i,"sigma"], nu=MM[i,"nu"], tau=MM[i,"tau"],  bd=MM[i,"bd"])  
             })  
      da[[i]] <- data.frame(y.var[[i]],  cdfArr[[i]])
    }
    da0 <- data.frame(y.var = from:to)
...
# loop over observations
  if (is(model,"gamlss"))
  {
    for (j in 1:lobs)
    {
      switch(nopar,
             {
               cdfArr[,j] <- 1-cdf(y.var,  mu = MM[j,2], lower.tail = lower.tail)
             },  
             {
               cdfArr[,j] <- 1-cdf(y.var, mu = MM[j,2],  sigma = MM[j,3], 
                                 lower.tail = lower.tail)
             },
             {
               cdfArr[,j] <- 1-cdf(y.var,  mu = MM[j,2], sigma = MM[j,3], 
  ...
  } else 
  {
    for (j in 1:lobs)
    {
      cdfArr[,j] <-  1-cdf(y.var,  par=predict(model, type="parameter")[obs[j],])  
    }  
  }  # end of look over observations       
################################################################
  da <- data.frame(y.var,  cdfArr)
...
################################################################################
################################################################################
fitted_survival_data <- function(model, obs, from, to,
                            colour = hcl.colors(length(obs), palette="viridis"), ...)  
{
  fitted_survival(model, obs=obs, from=from, to=to, ...)+
    ggplot2::geom_vline(xintercept = model$y[obs],  colour=colour[1:length(obs)])
}
